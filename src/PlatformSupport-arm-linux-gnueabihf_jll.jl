module PlatformSupport-arm-linux-gnueabihf_jll
...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import PlatformSupport-arm-linux-gnueabihf_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("20d984ba-9f33-5c50-9cf6-62d7ff8ee4f3"))
platforms = [Pkg.Artifacts.unpack_platform(e, "PlatformSupport-arm-linux-gnueabihf", artifacts_toml) for e in artifacts["PlatformSupport-arm-linux-gnueabihf"]]
...
if best_platform === nothing
    @debug("Unable to load PlatformSupport-arm-linux-gnueabihf; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module PlatformSupport-arm-linux-gnueabihf_jll
